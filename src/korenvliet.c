...
static void display_location(void)
{
    ...
    printf("\n---\n");
    printf("Plaats    : %s.\n", loc[player_location].desc);
    printf("Uitgangen : ");
    n_exits = 0;
    for (x = 0; x < MAX_EXITS; x++) {
        if (loc[player_location].exits[x].dir != '-') {
            if (n_exits > 0) printf(" ");
            print_dir_name(loc[player_location].exits[x].dir);
            printf(".");
            n_exits++;
        }
    }
    ...
    printf("U ziet    : ");
    if (obj[OBJ_ZWEMBRIL].loc != 0 && (player_location == LOCATION_ONDERWATER_30 || player_location == LOCATION_ONDERWATER_31)) {
    ...
    if (obj[OBJ_ZWEMBRIL].loc == 0 && player_location == LOCATION_ONDERWATER_31)
        printf("Een tunnel onder water.\n");
    if ((player_location == LOCATION_HUISKAMER && grate1) || (player_location == LOCATION_STUDEERKAMER && grate2) || (player_location == LOCATION_ATRIUM && grate3) || (player_location == LOCATION_WIJNKELDER_WEST && grate4))
        printf("putdeksel. ");
    ...
    /* Random atmospheric descriptions */
    {
        ...
        for (ai = 0; ai < (int)(sizeof(at)/sizeof(at[0])); ai++) {
            if (player_location == at[ai].room &&
                ((at[ai].cmp == '=' && z == at[ai].zval) ||
                 (at[ai].cmp == '<' && z < at[ai].zval)))
                printf("%s\n", at[ai].text);
        }
        if (player_location == LOCATION_VIJVER && obj[OBJ_ZALM].loc == 0 && z < GULL_THRESH)
...
static int find_obj_by_name(const char *name, int max_idx)
{
    ...
    for (x = 1; x <= max_idx; x++) {
        ...
        if (g > 0 && strcmp(name + nlen - g, obj[x].short_name + slen - g) == 0)
            return x;
    }
    return 0;
}
...
{
    if (strncmp(arg, "pak ", 4) == 0)
        memmove(arg, arg + 4, strlen(arg) - 3);
    else if (strncmp(arg, "neem ", 5) == 0)
        memmove(arg, arg + 5, strlen(arg) - 4);

    /* Specific checks before generic handling */
    if (strcmp(arg, "zalm") == 0 && player_location == LOCATION_ZUIDBAAI && obj[OBJ_VISNET].loc != 0) {
        printf("Die glipte uit Uw vingers.\n");
        return RET_KEEP;
    }
    if (strcmp(arg, "schilderij") == 0 && player_location == LOCATION_OVERLOOP) {
        printf("Te kostbaar.\n");
        return RET_KEEP;
    }
    if (player_location == LOCATION_SUPERMARKT) {
        printf("Pleeg geen winkeldiefstal!\n");
        return RET_KEEP;
    }
    if (strcmp(arg, "tafel") == 0 && player_location == LOCATION_SCHUUR) {
    ...
    /* Generic take */
    {
        ...
        if (idx == 0) {
            if (strcmp(arg, "panter") == 0 && obj[OBJ_PANTER].loc == player_location) {
                printf("U had nog net genoeg kracht om\nweg te komen.\n");
                health_status = SICK; player_location = LOCATION_ZIEKENHUIS;
                return RET_REDRAW;
            }
            if (strcmp(arg, "klok") == 0 && player_location == LOCATION_STUDEERKAMER) {
                printf("Te zwaar.\n"); return RET_KEEP;
            }
            if (strcmp(arg, "kist") == 0 && obj[OBJ_KIST].loc == player_location) {
                printf("Ik heb geen dorst.\n"); return RET_KEEP;
            }
            if (strcmp(arg, "kluis") == 0 && obj[OBJ_KLUIS].loc == player_location) {
...
static int cmd_drop(char *arg)
{
    ...
    /* "leg snorkel" special */
    if (strncmp(arg, "leg snorkel", 11) == 0) {
        ...
        arg += 4;
    {
        ...
        for (x = 1; x <= MAX_INV; x++) {
            ...
            int g = (int)arglen;
            if (g > (int)slen) g = (int)slen;
            if (g > 0 && strcmp(arg + arglen - g, obj[x].short_name + slen - g) == 0 && obj[x].loc == 0) {
...
static int cmd_go(const char *arg)
{
    ...
    if (strncmp(arg, "ga ", 3) == 0)
        arg += 3;
    else if (strncmp(arg, "ga", 2) == 0)
        arg += 2;

    dir = (char)tolower((unsigned char)*arg);

    if (strncmp(arg, "noord", 5) == 0) dir = 'n';
    else if (strncmp(arg, "oost", 4) == 0) dir = 'o';
    else if (strncmp(arg, "zuid", 4) == 0) dir = 'z';
    else if (strncmp(arg, "west", 4) == 0) dir = 'w';
    else if (strncmp(arg, "uit", 3) == 0) dir = 'u';
    else if (strncmp(arg, "omhoog", 6) == 0) dir = 'h';
    else if (strncmp(arg, "omlaag", 6) == 0) dir = 'l';
    ...
    for (x = 0; x < MAX_EXITS; x++) {
        if (loc[player_location].exits[x].dir == dir) {
            player_location = loc[player_location].exits[x].dest;
            return RET_REDRAW;
        }
    }
...
static int cmd_enter(const char *arg)
{
    ...
    if (strncmp(arg, "ga in ", 6) == 0)
        place = arg + 6;
    else if (strncmp(arg, "ga in", 5) == 0)
        place = arg + 5;
    else
        return RET_NOT_MINE;
    ...
    if (strcmp(place, "afvoer") == 0) {
        ...
        if ((player_location == LOCATION_HUISKAMER && grate1 == 0) || (player_location == LOCATION_STUDEERKAMER && grate2 == 0) ||
        ...
        if (player_location == LOCATION_WIJNKELDER_WEST && grate4) { player_location = LOCATION_RIOOL_27; return RET_REDRAW; }
        return fail();
    }

    if (strcmp(place, "ballon") == 0) {
    ...
    if (strcmp(place, "winkel") == 0 || strcmp(place, "supermarkt") == 0) {
        if (player_location != LOCATION_HOOFDSTRAAT) return fail();
        {
            int x;
            for (x = 1; x <= MAX_INV; x++) {
                if (obj[x].loc == 0) {
        ...
        player_location = LOCATION_SUPERMARKT; return RET_REDRAW;
    }

    if (strcmp(place, "landhuis") == 0 || strcmp(place, "korenvliet") == 0) {
        if (player_location == LOCATION_HOOFDSTRAAT)  { player_location = LOCATION_FOYER; return RET_REDRAW; }
        if (player_location == LOCATION_BINNENPLEIN)  { player_location = LOCATION_ATRIUM; return RET_REDRAW; }
        return fail();
    }

    if (strcmp(place, "ziekenhuis") == 0) {
        if (player_location == LOCATION_HOOFDSTRAAT) { player_location = LOCATION_ZIEKENHUIS; return RET_REDRAW; }
        return fail();
    }

    if (strcmp(place, "tunnel") == 0) {
        if (player_location == LOCATION_ONDERWATER_31 && obj[OBJ_ZWEMBRIL].loc == 0) { player_location = LOCATION_STROOM; return RET_REDRAW; }
        return fail();
    }

    if (strcmp(place, "kanaal") == 0) {
    ...
    if (strcmp(place, "afgraving") == 0) {
        if (player_location == LOCATION_AFGRAVING) { printf("Te steil.\n"); return RET_KEEP; }
        return fail();
    }

    if (strcmp(place, "schuur") == 0) {
        if (player_location == LOCATION_PLATEAU) { player_location = LOCATION_SCHUUR; return RET_REDRAW; }
        return fail();
    }
...
static int cmd_examine(const char *arg)
{
    ...
    if (strncmp(arg, "onderzoek ", 10) == 0)
        item = arg + 10;
    else if (strncmp(arg, "bekijk ", 7) == 0)
        item = arg + 7;
    else if (strncmp(arg, "open ", 5) == 0)
        item = arg + 5;
    else if (strncmp(arg, "open", 4) == 0)
        item = arg + 4;
    else
        return RET_NOT_MINE;
    ...
    x = 0;
    {
        ...
        for (xi = 1; xi <= MAX_OBJ; xi++) {
            ...
            g = (int)arglen;
            if (g > (int)slen) g = (int)slen;
            if (g > 0 && strcmp(item + arglen - g, obj[xi].short_name + slen - g) == 0 &&
                (obj[xi].loc == player_location || obj[xi].loc == 0))
                { x = xi; break; }
        }
    }
    if (x == 0) return fail();

    if (strcmp(obj[x].short_name, "fles") == 0) {
        printf("%s %s\n", pref[2], n[0]); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "beker") == 0) {
        printf("%s %s\n", pref[2], n[1]); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "tafel") == 0) {
        printf("Er ligt een briefje met het nummer %s\n", n[2]); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "kist") == 0) {
        printf("Er ontbreekt een fles.\n"); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "boek") == 0) {
    ...
    if (strcmp(obj[x].short_name, "klok") == 0 && obj[OBJ_ZWEMBRIL].loc == OBJECT_GONE) {
        printf("Er zit een duikbril in.\n"); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "tas") == 0 && obj[OBJ_SNORKEL].loc == OBJECT_GONE) {
        printf("Er zit een snorkel in.\n"); return RET_KEEP;
    }
    if (strcmp(obj[x].short_name, "schilderij") == 0) {
        printf("Er zit een kluis achter!\n");
        painting_examined = 1;
        obj[OBJ_KLUIS].loc = player_location;
        return RET_KEEP;
    }
    printf("Niets bijzonders.\n");
    return RET_KEEP;
...
static int cmd_go_up(void)
{
    ...
    for (x = 0; x < MAX_EXITS; x++) {
        if (loc[player_location].exits[x].dir == 'u') {
            player_location = loc[player_location].exits[x].dest;
            return RET_REDRAW;
        }
    }
...
{
    if (player_location == LOCATION_BOS && (obj[OBJ_BIJL].loc == 0 || obj[OBJ_BIJL].loc == player_location)) {
...
{
    if (player_location == LOCATION_VIJVER || player_location == LOCATION_ZUIDBAAI) {
        if (obj[OBJ_SNORKEL].loc == 0) {
            player_location += WATER_OFF;
            return RET_REDRAW;
        }
...
{
    if (player_location == LOCATION_HUISKAMER || player_location == LOCATION_RIOOL_21) { grate1 = 1; return RET_REDRAW; }
    if (player_location == LOCATION_STUDEERKAMER || player_location == LOCATION_RIOOL_24) { grate2 = 1; return RET_REDRAW; }
    if (player_location == LOCATION_ATRIUM || player_location == LOCATION_RIOOL_26) { grate3 = 1; return RET_REDRAW; }
    if (player_location == LOCATION_WIJNKELDER_WEST || player_location == LOCATION_RIOOL_27) { grate4 = 1; return RET_REDRAW; }
    return fail();
}
...
static int cmd_open_safe(void)
{
    ...
    if (painting_examined == 0) { printf("U kunt het niet vinden.\n"); return RET_KEEP; }
    if (player_location != LOCATION_OVERLOOP) { printf("Is hier niet.\n"); return RET_KEEP; }
...
static int cmd_fly_balloon_built(void)
{
    ...
    if (player_location == LOCATION_BALLON_PLATEAU) {
        ...
        player_location = LOCATION_BALLON_STRAAT; return RET_REDRAW;
    }
    if (player_location == LOCATION_BALLON_STRAAT) {
        for (y = BALLOON_Y_START; y <= BALLOON_Y_END; y += BALLOON_Y_STEP) {
            z = BALLOON_MIN_ALT + abs(BALLOON_ASCENT * y - BALLOON_MID_Y * BALLOON_ASCENT) / BALLOON_DIVISOR;
            printf("Ballon op hoogte %d...\n", z);
        }
        player_location = LOCATION_BALLON_PLATEAU; return RET_REDRAW;
    }
    return fail();
}
...
{
    if (obj[OBJ_BORD].loc == 0 || obj[OBJ_BORD].loc == player_location) {
...
static int handle_command(const char *cmd)
{
    ...
    /* "koop " at supermarkt */
    if (strncmp(cp, "koop ", 5) == 0 && player_location == LOCATION_SUPERMARKT) {
        char *item = cp + 5;
        while (*item == ' ') item++;
...
static void init_data(void)
{
    ...
    {
        /* Must have exactly MAX_OBJ (33) entries — keep in sync */
    ...
    {
        /* Must have exactly MAX_LOC (37) entries — keep in sync */
        const char *loc_desc[] = {
            "op het binnenplein",
            "in een bos",
            "in een weiland",
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (fgets(input, sizeof(input), stdin) == NULL)
                break;
            chomp(input);
            { char *p = input; while (*p) { *p = (char)tolower((unsigned char)*p); p++; } }
