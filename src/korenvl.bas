001 rem '  Implementation of the BASICODE API, versions 2, 3 and 3C.
002 rem '  Lines 1-999 are:
003 rem '  - Copyright (c) 2015, 2016 Rob Hagemans
004 rem '  - Released under the Expat MIT licence [https://opensource.org/licenses/MIT]
005 rem '
...
009 GOTO 21
010 rem '
011 rem ' Settings and initialisation
012 rem '
013 screem 7,0,0: KEY OFF: LOCATE 25,1: PRINT"                                                                       "
020 GOTO 1010 : rem ' Programs should start with A=...:GOTO 20 but some do other things there
021 CT = 0: CN = 0: SR = 0: SR$ = "": IN$ = " "
025 CC[0] = 7: CC[1] = 0
029 rem ' Colour map
030 O.CL%[0] = 0: O.CL%[1] = 1: O.CL%[2] = 4: O.CL%[3] = 5
031 O.CL%[4] = 2: O.CL%[5] = 3: O.CL%[6] = 14: O.CL%[7] = 15
032 rem ' Determine available graphics screen
033 ON ERROR GOTO 50

037 O.SCRN% =   7: HG = 320: VG = 200: O.WD% = 40: SCREEN O.SCRN%: IF O.SCRN% > 0 THEN 51

050 O.SCRN% =   0: RESUME NEXT
051 ON ERROR GOTO 0
058 rem ' Initialise text screen
059 O.HG% = 24: HO = O.WD% - 1: VE = O.HG% - 1
060 GOSUB 100
061 rem ' Determine if we have PCjr/Tandy sound capabilities
062 ON ERROR GOTO 64
063 O.PCJR% = -1: SOUND ON: GOTO 65
064 O.PCJR% = 0: RESUME 65
065 ON ERROR GOTO 0
066 PLAY "MF": SV = 7
070 rem ' Disable function key macros
075 FOR O.I% = 1 TO 10
076   KEY O.I%, ""
077 NEXT
080 RANDOMIZE TIMER
085 DIM O.FN%[8]: O.FFN% = 0
090 GOTO 1000 : rem ' Yield control to client
097 rem '
098 rem ' BASICODE-2 subroutines
099 rem '
100 rem ' Initialise text screen
101   screen 7: rem SCREEN 0, 0, 0, 0
102   WIDTH O.WD%
103   KEY OFF
104   rem COLOR O.CL%[CC[0]],O.CL%[CC[1]],0
105   CLS
109 RETURN
110 rem ' Set cursor position
111   IF (HO < 0) OR (HO >= O.WD%) OR (VE < 0) OR (VE >= O.HG%) THEN RETURN
112   LOCATE VE+1, HO+1
113 RETURN
120 rem ' Retrieve cursor position
121   VE = CSRLIN-1
122   HO = POS(0)-1
123 RETURN
150 rem ' Print highlighted text block to screen
151   COLOR O.CL%[CC[0]], O.CL%[CC[1]]: PRINT " ";
152   COLOR O.CL%[CC[1]], O.CL%[CC[0]]: PRINT "   "; SR$; "   ";
153   COLOR O.CL%[CC[0]], O.CL%[CC[1]]: PRINT " ";
154 RETURN
200 rem ' Read keystroke
201   IN$ = INKEY$: IF IN$ = "" THEN IN = 0: RETURN
202   IN = ASC(IN$): IF IN >= ASC("a") AND IN <= ASC("z") THEN IN = IN - 32: in$=chr$(in)
...
209 RETURN
210 rem ' Wait for keystroke
211     GOSUB 200
212   IF IN$ = "" AND IN = 0 THEN GOTO 211
213 RETURN
220 rem ' Read character from screen
221   IN = SCREEN(VE+1, HO+1): CN = 0
222   IF IN >= ASC("a") AND IN <= ASC("z") THEN IN = IN-32: CN = 32
229 RETURN
250 rem ' Beep the speaker
251   BEEP
252 RETURN
260 rem ' Obtain a random number
261   RV = RND(1)
262 RETURN
270 rem ' Perform garbage collection and get amount of free memory
271   FR = FRE("")
272 RETURN
280 rem ' Switch break key on/off
281   KEY 20, CHR$(&H04)+CHR$(&H46)
285   ON KEY(20) GOSUB 290
287   IF FR = 1 THEN KEY(20) ON ELSE KEY(20) OFF
290 RETURN
300 rem ' Format number as string
301   SR$ = STR$(SR)
302   IF LEFT$(SR$,1) = " " THEN SR$ = MID$(SR$, 2, LEN(SR$)-1)
303 RETURN
310 rem ' Format number as string; length and decimal places specified
311   O.IN$ = STR$(FIX(SR)): O.FR$ = STR$(SR-FIX(SR))
312   IF LEFT$(O.IN$, 1) = " " THEN O.IN$ = RIGHT$(O.IN$, LEN(O.IN$)-1)
...
320   NEXT
321   IF LEN(O.FR$) < CN+1 THEN O.FR$ = O.FR$ + STRING$(CN+1-LEN(O.FR$), "0")
322   SR$ = O.IN$ + O.FR$: IF LEN(SR$) > CT THEN SR$=STRING$(CT, "*"): RETURN
324   SR$ = STRING$(CT-LEN(SR$), " ") + SR$
326 RETURN
330 rem ' Capitalise a string (BASICODE-3)
331   FOR O.I = 1 TO LEN(SR$)
332     IF MID$(SR$, O.I) >= "a" AND MID$(SR$, O.I) <= "z" THEN MID$(SR$, O.I) = CHR$(ASC(MID$(SR$, O.I)) - &H20)
334   NEXT
336 RETURN
350 rem ' Print string to printer
351   LPRINT SR$;
352 RETURN
360 rem ' Print carriage return/newline to printer
361   LPRINT
362 RETURN
397 rem '
398 rem ' BASICODE-3 subroutines
399 rem '
400 rem ' Produce a tone on the speaker of specified frequency and duration
...
405 RETURN
450 rem ' Wait for SD/10 seconds or until a key is pressed
...
456 RETURN
480 rem ' Get file number for NF, use NF=0 for first free file (internal)
...
488 RETURN
490 rem ' File error handler (internal)
491   IN = -1
492 RESUME NEXT
500 rem ' Open a file
...
546   ON ERROR GOTO 0
547 RETURN
560 rem ' Write a string to file
562   GOSUB 480
...
566   ON ERROR GOTO 0
567 RETURN
580 rem ' Close the file
581   GOSUB 480
582   CLOSE O.FFN%
583   O.FN%[O.FFN%] = 0
582 RETURN
600 rem ' Initalise graphics screen
601   SCREEN O.SCRN%
603   COLOR O.CL%[CC[0]], O.CL%[CC[1]]
604   CLS
605   WINDOW SCREEN (0, 0) - (1, 1)
606 RETURN
620 rem ' Put a pixel on the screen
621   PSET (HO, VE), O.CL%[CC[CN]]
625 RETURN
630 rem ' Draw a line on the screen
631   LINE -(HO, VE), O.CL%[CC[CN]]
635 RETURN
650 rem ' Draw text at a specified position on the graphics screen
651   DEF SEG = &HF000
652   O.HO% = PMAP(HO, 0): O.VE% = PMAP(VE, 1)
655   WINDOW
657   rem 'LINE (O.HO%, O.VE%)-(O.HO% + 8*LEN(SR$)-1, O.VE%+7), O.CL%[CC[1-CN]], BF
...
682 RETURN
950 rem ' Finalise BASICODE
951   CLOSE
952   CC[0] = 7: CC[1] = 0: GOSUB 100
954   PALETTE
955 END
996 rem '
997 rem '  End of BASICODE API implementation.
998 rem '  Line 1000 onwards form part of a BASICODE program.
999 rem '
...
1085   IF POS(0) + LEN(O$(X)) > 38 THEN :PRINT: PRINT TAB(13);
1090   PRINT O$(X)". ";
1100 NEXT :PRINT :PRINT: GOSUB 5200
1190 rem
1192 print: print"Wat nu    : ";:INPUT"",C$: PRINT
...
1310 goto 2750 : rem einde GA IN
...
2189 rem: "LEG"
2190 FOR X =1 TO 19: H = INSTR(5,C$," ")
...
2779 rem: Mag ik naar de  winkel ?
...
7790 WL$=INKEY$: if wl$="" then goto 7790
7800 GOTO 1040
...
9610 H$=CHR$(12):
9620 DEFINT B-Z: DIM I$(33),O$(33),O(33),L$(37),D$(3,37),D(3,37): L=9: I=0:
...
9647 Y = 0
9648 FOR X = 1 TO 3: z = INT(RND*(90+1))+10 : N$(X) =STR$(Z): print n$(x) : NEXT
9650 FOR X = 1 TO 3
9660   Z = INT(3*RND(5))+1:
9661   IF S(Z) = Z THEN goto 9660
9662   S$(Z) = RIGHT$ (N$(X),2)
9663   S(Z) = Z
9664   print s$(z)
9665 NEXT
9666 for i = 1 to 30000: next : rem wait 3 seconds
9670 rem
