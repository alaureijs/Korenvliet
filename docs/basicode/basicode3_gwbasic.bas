1 rem BASICODE 3 routines voor IBM / GW BASIC(A).
2 rem (C) Copyright B. Niewold 1984,85 (BC2 routines)
3 rem (C) Copyright J. Goudsmit 1987 (BC3 aanvullingen)                             '
4 rem De BC3 routines mogen vrij gekopieerd worden, mits voor niet-commerciele      ' doeleinden.                                                                   '
5 rem Voor Basicode 3 heeft U BASICA versie 2.0 of hoger nodig, evenals de bij-     ' behorende DOS-versie. Dit i.v.m. enkele BASICA opdrachten die pas vanaf
6 rem versie 2.0 geimplementeerd zijn. Disk BASIC is niet bruikbaar, omdat daar     ' enkele grafische en andere routines niet inzitten.
7 rem Voor mooie grafieken hebt U een Color/Graphics adapter of een Hercules        ' Monochrome Graphics kaart nodig.
8 rem Vanwege de ingewikkelde procedure, en omdat het oplossend vermogen daartoe      meestal toch niet toereikend is, is er geen routine geimplementeerd voor        lijnen op het monochrome beeldscherm. Punten mogen wel, evenals tekst.
9 RANDOMIZE TIMER: KEY OFF: LOCATE 25,1: PRINT"                                                                       "

10 GOTO 1000
11 rem'In regel 20 worden de variabelen geinitialiseerd voor resp.:                    voor- en achtergrondkleur, border-kleur, accent-voor- en -achtergrondkleuren    breedte, mode (zie 23-), palletkleur en grafische kleur
20 OFG=7:OBG=0:OBD=0:OAF=14:OAB=4:OWD=40:OGS=2:OPL=0:OGK=2: rem                       'Deze regel naar smaak aanpassen. Tip: gebruik ENVIRON$ om deze waarden in te    lezen.
21 VE=23:HO=OWD-1:IN$="":IN=0:SR$="":CT=0:CN=0:OGF=1 : rem 'ogf= flag voor grafmode
22 KEY OFF:FOR O=1 TO 10:KEY O,"":KEY(O) OFF:NEXT:GOSUB 100
23 ON OGS+1 GOTO 24,25,26,27
24 HG=80:VG=24:GOTO 28   :rem 'Geen grafische mogelijkheid: gebruik maken van tekst-scherm.
25 HG=320:VG=192:GOTO 28 :rem 'Gebruik maken van SCREEN 1 en COLOR BACKG,PALET
26 HG=640:VG=192:GOTO 28 :rem 'SCREEN 2 en voorgrondkleur BACKG. Merk op dat het aspect in deze mode flink afwijkt van de 3/4 die door BC3 is aangeraden.
27 HG=720:VG=348 : rem 'SCREEN 2 wordt gebruikt, maar er wordt rekening gehouden met de 720x348 resolutie van het Hercules scherm.
...
100 CLS:IF OGF THEN OUT &H3D9,0:SCREEN 7:WIDTH OWD:COLOR OFG,OBG,OBD:OGF=0:LOCATE ,,0:CLS
105 RETURN
...
204 RETURN : rem ' Merk op dat afdrukken van codes 28-31 WEL werkt op de IBM
...
281 RETURN : rem 'Ook gebruikt voor ON KEY(15) GOSUB
...
360 LPRINT:RETURN
400 PLAY"MFML":IF SV<=0 THEN SOUND 32767,SD*1.82:RETURN : rem 'Alleen volumes >0 spelen
401 OSD=SD:IF OSD<1 THEN OSD=1
402 ONM=1:WHILE OSD>64:OSD=OSD/2:ONM=ONM*2:WEND:PLAY"T38L"+STR$(INT(64/OSD+.5)) : rem 'Door die 38 ipv 37.5 en die INT gaat de werkelijke tijd al snel afwijken.
...
936 RESUME 'Retry
937 RESUME NEXT : rem 'Ignore error
938 ON ERROR GOTO 0:RESUME : rem 'Retry error - geeft BASIC melding
939 RESUME 950 : rem 'Abort - zonder boodschap
